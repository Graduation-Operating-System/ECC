...
// AES CBC 모드 암호화
void aes_cbc_encrypt(const uint8_t *plaintext, uint8_t *ciphertext, const uint8_t *key, uint8_t *iv, size_t length) {
    uint8_t block[AES_BLOCK_SIZE];

    for (size_t i = 0; i < length; i += AES_BLOCK_SIZE) {
        // 현재 블록과 IV 또는 이전 블록의 암호문 XOR
        for (int j = 0; j < AES_BLOCK_SIZE; j++) {
            if (i == 0) {
                block[j] = plaintext[i + j] ^ iv[j]; // 첫 블록은 IV와 XOR
            } else {
                block[j] = plaintext[i + j] ^ ciphertext[i - AES_BLOCK_SIZE + j]; // 이후 블록은 이전 암호문과 XOR
            }
        }
        aes_encrypt(block, ciphertext + i, key); // 암호화
    }
}
